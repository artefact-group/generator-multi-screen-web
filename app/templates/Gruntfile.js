...
module.exports = function (grunt) {
  ...
  grunt.initConfig({
    ...
    compress: {
      ...
      finalWindowsApp: {
        options: {
          archive: '<%= config.distWin %>/<%%= name %%>.zip'
        },
        files: [{
          expand: true,
          cwd: '<%= config.tmp %>',
    ...
    rename: {
      macApp32: {
        files: [{
          ...
          dest: '<%= config.distMac32 %>/<%%= name %%>.app'
        }]
      },
      macApp64: {
        files: [{
          src: '<%= config.distMac64 %>/node-webkit.app',
          dest: '<%= config.distMac64 %>/<%%= name %%>.app'
        }]
      },
      zipToApp: {
        files: [{
  ...
  grunt.registerTask('dmg', 'Create dmg from previously created app folder in dist.', function () {
    var done = this.async();
    var createDmgCommand = 'resources/mac/package.sh "<%%= name %%>"';
